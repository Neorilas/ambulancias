...
| Pantalla | `/flota` → `pages/flota/MapaFlota.jsx` + `components/flota/MapaLeaflet.jsx`. `?vehiculo=<id>` abre con ese vehículo seleccionado (desde la ficha: tarjeta «Asignada/Libre» y botón «Ver en el mapa»). Se aplica **una sola vez**, con la primera carga: reaplicarlo en cada refresco de 30 s haría saltar el mapa aunque el usuario ya mirase otro. Si no tiene posición, baja hasta su ficha (en el móvil queda bajo el mapa y parecería que el enlace no hizo nada) |
