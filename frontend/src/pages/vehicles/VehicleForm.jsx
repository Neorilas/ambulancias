  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Nombre y matrícula. El nombre va primero y ocupa toda la fila: es
            el identificador que usa el personal y el que ordena la flota. */}
        <div>
          <label className="label">Nombre de la ambulancia <span className="text-bad-500">*</span></label>
          <input
            type="text"
            className={`input ${errors.alias ? 'input-error' : ''}`}
            value={form.alias}
            onChange={set('alias')}
            placeholder="Ej: Ambulancia 1"
            autoFocus={!isEdit}
          />
          {errors.alias
            ? <p className="field-error">{errors.alias}</p>
            : <p className="hint">Como se conoce el vehículo en el día a día. Es lo que se ve en los listados.</p>}
        </div>
